...
tar_gt = './dat/extra_GT1.csv'

fd_out = '../data/extra_all1'
...
for img_fn in tqdm(flist):    
    ...
    elif Path(img_fn).parts[-2]  =='皮角': #set to AK
        fn_gts.append([Path(str_id).stem ,3])
    else: # unknown now can map to benign
...
for fn0,dg in zip(tqdm(fns),diagnosis):    
    ...
    if dg in [0,1]:  # a modification that type=1 is  used here
        fn_gts.append([Path(str_id).stem ,   1])
        tp = 'NV'
    elif dg==2:
        fn_gts.append([Path(str_id).stem ,   0])
