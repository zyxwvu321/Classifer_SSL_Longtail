...
__all__ = ['mobilenetv2_1dot0', 'mobilenetv2_1dot0_fc512','mobilenetv2_1dot0_bnn_fc512','mobilenetv2_1dot0_bnn_fc512_drp05',
           'mobilenetv2_1dot0_bnn_fc256_drp025','mobilenetv2_1dot0_bnn_fc512_drp025','mobilenetv2_1dot4']
...
model_fns = {
    # 1.0: top-1 71.3
    'mobilenetv2_1dot0': '../checkpoints/mobilenetv2_1.0-0f96a698.pth',
    # 1.4: top-1 73.9
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes, width_mult=1, loss='softmax', fc_dims=None, dropout_p=None, last_stride = 1,**kwargs):
...
class MobileNetV2_bnn(nn.Module):
    """MobileNetV2.
    ...
    def __init__(self, num_classes, width_mult=1, loss='softmax', fc_dims=None, dropout_p=None, **kwargs):
        super(MobileNetV2_bnn, self).__init__()
        self.loss = loss
        self.in_channels = int(32 * width_mult)
        self.feature_dim = int(1280 * width_mult) if width_mult > 1 else 1280
        ...
        self.global_avgpool = nn.AdaptiveAvgPool2d(1)
        self.bnn            =  nn.BatchNorm1d(self.feature_dim)
    ...
    def _construct_fc_layer(self, fc_dims, input_dim, dropout_p=None):
        ...
        layers = []
        for dim in fc_dims:
            layers.append(nn.Linear(input_dim, dim,bias = False))
            #layers.append(nn.BatchNorm1d(dim))
            #layers.append(nn.ReLU(inplace=True))
            if dropout_p is not None:
                layers.append(nn.Dropout(p=dropout_p))
    ...
    def forward(self, x):
        ...
        if self.loss == 'softmax':
            ...
            raise KeyError("Unsupported loss: {}".format(self.loss))
def init_pretrained_weights(model, model_fn):
    """Initializes model with pretrained weights.
    
    Layers that don't match with pretrained layers in name or size are kept unchanged.
    """
    #pretrain_dict = model_zoo.load_url(model_url)
    pretrain_dict = torch.load(model_fn)
    model_dict = model.state_dict()
    pretrain_dict = {k: v for k, v in pretrain_dict.items() if k in model_dict and model_dict[k].size() == v.size()}
    model_dict.update(pretrain_dict)
    model.load_state_dict(model_dict)
    print('Initialized model with pretrained weights from {}'.format(model_fn))


def mobilenetv2_1dot0(num_classes, loss='softmax', pretrained=True, last_stride = 1,**kwargs):
    model = MobileNetV2(
        num_classes,
        loss=loss,
        width_mult=1,
        fc_dims=None,
        dropout_p=None,last_stride = last_stride,
        **kwargs
    )
    if pretrained:
        init_pretrained_weights(model, model_fns['mobilenetv2_1dot0'])
    return model

def mobilenetv2_1dot0_fc512(num_classes, loss, pretrained=True, **kwargs):
    model = MobileNetV2(
        num_classes,
        loss=loss,
        width_mult=1,
...
def mobilenetv2_1dot0_bnn_fc512(num_classes, loss, pretrained=True, **kwargs):
    model = MobileNetV2_bnn(
        num_classes,
        loss=loss,
        width_mult=1,
        fc_dims=[512],
...
def mobilenetv2_1dot0_bnn_fc512_drp05(num_classes, loss, pretrained=True, **kwargs):
    model = MobileNetV2_bnn(
        num_classes,
        loss=loss,
        width_mult=1,
    ...
    if pretrained:
        init_pretrained_weights(model, model_fns['mobilenetv2_1dot0'])
    return model
def mobilenetv2_1dot0_bnn_fc256_drp025(num_classes, loss, pretrained=True, **kwargs):
    model = MobileNetV2_bnn(
        num_classes,
        loss=loss,
        width_mult=1,
...
def mobilenetv2_1dot0_bnn_fc512_drp025(num_classes, loss, pretrained=True, **kwargs):
    model = MobileNetV2_bnn(
        num_classes,
        loss=loss,
        width_mult=1,
...
def mobilenetv2_1dot4(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MobileNetV2(
        num_classes,
        loss=loss,
        width_mult=1.4,
    ...
    if pretrained:
        init_pretrained_weights(model, model_fns['mobilenetv2_1dot4'])
    return model
