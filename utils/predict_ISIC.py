...
if __name__ == '__main__':
    ...
    if  args.is_valid ==1:
        ...
        norm_cm = cm/np.sum(cm,axis = 1,keepdims = True)
        norm_cm = np.round_(norm_cm,decimals=4)
        acc = np.sum(cm.diagonal())/cm.sum()
        acc_bal = np.mean(norm_cm.diagonal())
        
        
        logger.info('confusion matix {}\n'.format(cm))
        logger.info('norm confusion matix {}\n'.format(norm_cm))
        
        logger.info(f'acc and acc_bal {acc :.4f} {acc_bal :.4f}\n')
